...
export const MarkdownRenderer = ({ content }: { content: string }) => {
  ...
  // Flat Pastel Styling
  processed = processed
    // Gambar (Rounded ekstrem, flat border, no heavy shadow)
    .replace(/!\[([^\]]*)\]\(([^)]+)\)/g, '<img src="$2" alt="$1" class="rounded-[2.5rem] w-full object-cover my-12 border border-border shadow-sm" />')
    // Tautan (Links)
    ...
    // Blockquotes (Flat solid pastel)
    .replace(/^> (.*$)/gim, '<blockquote class="border-l-4 border-primary pl-6 pr-4 italic text-muted-foreground my-10 py-6 bg-secondary rounded-r-[2rem] text-lg leading-relaxed">$1</blockquote>')
    // Baris Baru (Newlines)
    .replace(/\n/gim, '<br />');

  // Kembalikan Blok Kode dengan gaya Flat (Solid background, no inner shadow)
  codeBlocks.forEach((block, i) => {
      const escapedBlock = block.replace(/</g, '&lt;').replace(/>/g, '&gt;');
      processed = processed.replace(
          `__CODE_BLOCK_${i}__`, 
          `<div class="relative group my-10"><pre class="bg-secondary p-6 md:p-8 rounded-[2.5rem] overflow-x-auto border border-border"><code class="font-mono text-sm md:text-base text-foreground/90 leading-relaxed">${escapedBlock}</code></pre></div>`
      );
  });
  ...
  useEffect(() => {
      ...
      preBlocks.forEach((block) => {
          ...
          btn.className = 'copy-btn absolute top-4 right-4 px-4 py-2 rounded-full bg-card border border-border text-muted-foreground hover:text-primary hover:border-primary/50 transition-all text-xs font-bold opacity-0 group-hover:opacity-100 shadow-sm flex items-center gap-2';
